...
with i as (
    SELECT i.account_id from subscriptions i 
        where extract(year from i.start_date)='2021' or
        extract(year from i.end_date)='2021'
)
select count(s.account_id) as accounts_count
from  i
left join streams s
on i.account_id=s.account_id
where extract(year from s.stream_date)!='2021' or s.account_id IS NULL;
