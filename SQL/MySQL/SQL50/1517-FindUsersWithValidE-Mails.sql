...
*/
# database ### Approach: Selecting rows based on conditions #### Algorithm In general, if you are asked to match a string,
  writing a regular expression pattern to match on should come first to mind. RegEx provides various functionality, 
  here are a few relevant ones: 1. ^: This represents the start of a string or line. 1. [a-z]: This represents a character range, 
  matching any character from a to z. * [0-9]: This represents a character range, matching any character from 0 to 9.
...
* [^a-z]: This variant matches any character that is not from a to z. Note that the ^ character is used to negate the character range, which means it has a different meaning inside the square brackets than outside where it means the start. 
  ...
  1. $: This represents the end of a string or line.

The key idea here is to separate the first character of the name column from the rest, change their cases accordingly, and then 
